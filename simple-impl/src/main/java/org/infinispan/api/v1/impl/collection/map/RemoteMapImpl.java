...
public class RemoteMapImpl<K, V> implements RemoteMap<K, V> {

   private final Queue<String> queue = new LinkedList<>();
   private final Map<K, V> data = new HashMap<>();
   private MapConfig config;

   public RemoteMapImpl(MapConfig config) {
      this.config = config;
   }
   ...
   public final static class FactoryImpl<K, V> implements Factory<K, V> {
      ...
      @Override
      public RemoteMap<K, V> apply(MapConfig mapConfig) {
         return new RemoteMapImpl<>(mapConfig);
      }
   }
}
